...
-- SUM(IFNULL(HistBatters.Hit,0))/NULLIF(SUM(IFNULL(HistBatters.atBat,0)),0)
...
select batter, sum(Hit)/sum(atBat+ 0.00001) as BA -- added 0.00001 to prevent division by zero error
from batter_counts bc 
group by batter 
order by BA desc ;
...
select a.game_id, a.batter, sum(b.Hit)/sum(b.atBat + 0.00001) as BA, count(*) as cnt, b.local_date
from   bc_join_g a
join   bc_join_g b
